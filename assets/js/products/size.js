$(document).ready(function () {
    ...
    function loadBrand(pageNumber = 0) {
        $.ajax({
            url: `http://127.0.0.1:8080/api/size/page?pageNumber=${pageNumber}`, // Đường dẫn API có hỗ trợ sắp xếp
            type: 'GET',
    ...
    function deleteProduct(Id) {
        if (confirm('Bạn có chắc chắn muốn xóa Sản Phẩm này?')) {
            ...
            $.ajax({
                url: `http://localhost:8080/api/size/${Id}`,  // API xóa người dùng
                type: 'DELETE',
    ...
    function openEditForm(Id) {
        ...
        $.ajax({
            url: `http://localhost:8080/api/size/${Id}`,
            method: 'GET',
